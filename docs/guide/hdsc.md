...
**源码:**
``` java
// efadmin-common -> vip.efactory.config.ElPermissionConfig
@Service(value = "el")
...
**efadmin-system -> modules -> security ->  config -> SecurityConfig**
``` java
// 关键代码，部分略
...
源码可以查看 `efadmin-common` 模块中的 `vip.efactory.annotation.Query` 与 `vip.efactory.utils.QueryHelp`
:::

## 系统缓存
本系统缓存使用的是 ```redis```，默认使用 ```Spring``` 的注解对系统缓存进行操作，并且提供了可视化的 ```redis``` 缓存操作
### 配置缓存
配置文件位于 ```efadmin-common``` 模块中的 `vip.efactory.config.RedisConfig`，配置文件如下：
``` java
@Slf4j
@Configuration
@EnableCaching
...
源码位于：源码位于：``` efadmin-common ``` 模块中的 `exception` 包中
...
本系统使用 ```AOP``` 方式记录用户操作日志，只需要在 ```controller``` 的方法上使用 ```@Log("")``` 注解，就可以将用户操作记录到数据库，源码可查看 ```efadmin-logging``` <br>
模块具体使用如下：
...
```
efadmin-common -> vip.efactory.base
```
...
要使用服务监控功能，需要将 efadmin-monitor-2.4.jar 部署到需要监控的服务器上

- Window服务器：请将eladmin\efadmin-monitor\src\main\resources目录下的sigar-x86-winnt.dll或者sigar-amd64-winnt.dll 放到java的bin目录下
- Linux服务器：请将eladmin\efadmin-monitor\src\main\resources目录下的libsigar-x86-linux.so或者libsigar-amd64-linux.so 放到java的bin目录下
- 将打包好的 efadmin-monitor-2.4.jar 放到服务器上，执行 nohup java -jar efadmin-monitor-2.4.jar >nohup.out 2>&1 & 即可
...
这里用 `efadmin-monitor-2.4.jar` 来举例，设置应用名称与目录，然后编写脚本即可
...
```shell script
mv -f /opt/upload/efadmin-monitor-2.4.jar /opt/monitor
cd /opt/monitor
nohup java -jar efadmin-monitor-2.4.jar >nohup.out 2>&1 &
```
...
```
efadmin-system -> vip.efactory.config.AsyncTaskExecutePool
```
...
```
efadmin-system -> vip.efactory.config.ThreadPoolExecutorUtil
```
